...
export class ComposeService {
  ...
  async createDeployment(userId: number, createComposeDto: CreateComposeDto) {
    ...
    const dockerSetup = await this.setupDocker(
      userId,
      createComposeDto.connectionUuid,
    );
    ...
    try {
      composeConfig = yaml.load(
        createComposeDto.composeContent,
      ) as ComposeConfig;
  ...
  private async validatePorts(serviceConfig: any): Promise<void> {
    ...
    for (const portMapping of serviceConfig.ports) {
      const [hostPort, containerPort] = portMapping
        .split(':')
        .map((p) => parseInt(p, 10));

      if (!(await this.isPortAvailable(hostPort, containerPort))) {
        throw new BadRequestException(
          `Port ${hostPort} (host) or ${containerPort} (container) is already in use. Please choose different ports.`,
        );
      }
    }
  }
  ...
  private async startDeployment(uuid: string) {
    ...
    if (!this.docker || this.currentConnection.id !== deployment.connectionId) {
      throw new ServiceUnavailableException(
        'Docker connection not properly initialized',
      );
    }
    ...
    for (const [serviceName, serviceConfig] of Object.entries(
      ...
      try {
        ...
        // Create container
        const containerConfig = this.convertComposeToContainerConfig(
          serviceName,
          serviceConfig,
        );
  ...
  private validateComposeConfig(config: ComposeConfig) {
    if (!config || !config.services || typeof config.services !== 'object') {
      throw new BadRequestException(
        'Invalid compose file: must contain services section',
      );
    }
  ...
  private convertComposeToContainerConfig(
    serviceName: string,
    serviceConfig: any,
  ) {
    const uniqueSuffix =
  ...
  async stopDeployment(userId: number, uuid: string) {
    ...
    for (const container of deployment.containers) {
      try {
        ...
        console.error(
          `Failed to stop container ${container.name}: ${error.message}`,
        );
      }
    }
