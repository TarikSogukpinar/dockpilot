...
export class ImageService {
  ...
  async pullImage(userId: number, createImageDto: CreateImageDto) {
    const { name, tag, registry, username, password, platform, forcePull } =
      createImageDto;

    // Setup Docker connection
    const dockerSetup = await this.setupDocker(
      userId,
      createImageDto.connectionId,
    );
    ...
    try {
      ...
      // Get image details
      const image = await this.docker.getImage(
        `${registry ? registry + '/' : ''}${name}:${tag}`,
      );
  ...
  async removeImage(
    ...
    try {
      ...
      return {
        success: true,
        message: `Image ${dbImage.name}:${dbImage.tag} removed successfully`,
      };
    } catch (error) {
  ...
  async tagImage(
    ...
    try {
      ...
      return {
        success: true,
        message: `Image ${dbImage.name}:${dbImage.tag} tagged as ${repo}:${tag}`,
      };
    } catch (error) {
