...
export class NetworkService {
    ...
    async createNetwork(userId: number, connectionUuid: string, networkConfig: any) {
    ...
    async listNetworks(userId: number, connectionUuid: string) {
        ...
        try {
            ...
            throw new BadRequestException(`Failed to list networks: ${error.message}`);
        }
    }

    async getNetwork(userId: number, connectionUuid: string, networkUuid: string) {
        const dockerSetup = await this.setupDocker(userId, connectionUuid);
        if (!dockerSetup.isConnected) {
            throw new ServiceUnavailableException(dockerSetup.error);
        }
    ...
    async removeNetwork(userId: number, connectionUuid: string, networkUuid: string) {
        const dockerSetup = await this.setupDocker(userId, connectionUuid);
        if (!dockerSetup.isConnected) {
            throw new ServiceUnavailableException(dockerSetup.error);
        }
        ...
        try {
            ...
            return { success: true, message: `Network ${dbNetwork.name} removed successfully` };
        } catch (error) {
            throw new BadRequestException(`Failed to remove network: ${error.message}`);
        }
    }

    async connectContainer(userId: number, connectionUuid: string, networkUuid: string, containerId: string, config?: any) {
        const dockerSetup = await this.setupDocker(userId, connectionUuid);
        if (!dockerSetup.isConnected) {
            throw new ServiceUnavailableException(dockerSetup.error);
        }
        ...
        try {
            ...
            return { success: true, message: `Container connected to network ${dbNetwork.name} successfully` };
        } catch (error) {
            throw new BadRequestException(`Failed to connect container to network: ${error.message}`);
        }
    }

    async disconnectContainer(userId: number, connectionUuid: string, networkUuid: string, containerId: string, force: boolean = false) {
        const dockerSetup = await this.setupDocker(userId, connectionUuid);
        if (!dockerSetup.isConnected) {
            throw new ServiceUnavailableException(dockerSetup.error);
        }
        ...
        try {
            ...
            return { success: true, message: `Container disconnected from network ${dbNetwork.name} successfully` };
        } catch (error) {
            throw new BadRequestException(`Failed to disconnect container from network: ${error.message}`);
        }
    }
    ...
    async pruneNetworks(userId: number, connectionUuid: string) {
        ...
        try {
            ...
            throw new BadRequestException(`Failed to prune networks: ${error.message}`);
        }
    }
}
