import { Controller, Get, Post, Body, Param, Query, UseGuards, ParseUUIDPipe, ParseEnumPipe } from '@nestjs/common';
...
export class ResourceController {
    ...
    async getMetrics(
        ...
        @Query('type', new ParseEnumPipe(ResourceMetricType)) type: ResourceMetricType,
        @Query('start') start: Date,
        @Query('end') end: Date,
    ) {
        return this.resourceService.getContainerMetrics(
            containerUuid,
            type,
            { start, end }
        );
    ...
    async setLimits(
        ...
        return this.resourceService.setResourceLimit(
            containerUuid,
            type,
            limits
        );
    ...
    async getAlerts(
        ...
        return this.resourceService.getContainerAlerts(
            containerUuid,
            resolved
        );
    }
    ...
    @ApiParam({ name: 'suggestionUuid', description: 'UUID of the optimization suggestion' })
    @ApiResponse({
        status: 200,
