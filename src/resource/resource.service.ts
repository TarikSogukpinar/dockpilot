...
export class ResourceService {
    ...
    async setResourceLimit(containerUuid: string, type: ResourceMetricType, limits: any) {
    ...
    private async checkAlerts(containerId: number, stats: any) {
        ...
        for (const limit of limits) {
            ...
            if (currentValue > limit.hardLimit) {
                await this.createAlert(containerId, limit.type, currentValue, limit.hardLimit, 'CRITICAL');
            } else if (currentValue > limit.softLimit) {
                await this.createAlert(containerId, limit.type, currentValue, limit.softLimit, 'WARNING');
            }
        }
    }
    ...
    private async generateOptimizationSuggestions(containerId: number, stats: any) {
    ...
    private isResourceUnderutilized(metrics: any[], type: string, threshold: number) {
    ...
    private isResourceOverutilized(metrics: any[], type: string, threshold: number) {
        const relevantMetrics = metrics.filter(m => m.type === type);
        const avgUsage = this.calculateAverage(relevantMetrics.map(m => m.value));
        return avgUsage > threshold;
    }
    ...
    // Query Methods
    async getContainerMetrics(containerUuid: string, type: ResourceMetricType, timeRange: { start: Date; end: Date }) {
        const container = await this.prisma.container.findUnique({
            where: { uuid: containerUuid }
        });
    ...
    async getOptimizationSuggestions(containerUuid: string, implemented?: boolean) {
        const container = await this.prisma.container.findUnique({
            where: { uuid: containerUuid }
        });
