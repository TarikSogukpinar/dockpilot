...
export class VolumeService {
  ...
  async removeVolume(
    ...
    try {
      ...
      return {
        success: true,
        message: `Volume ${dbVolume.name} removed successfully`,
      };
    } catch (error) {
      throw new BadRequestException(
        `Failed to remove volume: ${error.message}`,
      );
    }
  }
  ...
  async pruneVolumes(userId: number, connectionUuid: string) {
    ...
    try {
      ...
      throw new BadRequestException(
        `Failed to prune volumes: ${error.message}`,
      );
    }
  }
